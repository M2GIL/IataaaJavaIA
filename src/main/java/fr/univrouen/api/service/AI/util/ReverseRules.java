...
public class ReverseRules {
    ...
    private static List<Piece[]> availableMoves = new ArrayList<>();
    private static int sizeMove = 1;

    // positionpieces est la liste des position pièces que l'on veut analyser.
    public static List<Piece[]> getAvailableMoves(Piece[] cases, PlayerNb p) {
        List<Integer> whiteCases;
        if (p == PlayerNb.PLAYER_2) {
        ...
        for (Piece[] c : avalaibleMovesList) {
            res.add(c);
        }
...
// Elle devront être privé quand les tests ne serviront plus.
    ...
    private static void fillAvalaibleMovesForOneCase(Piece[] pieces, int srcPosition) {
        ...
        if (!isPossibleToJump && sizeMove == 1) { // S'il n'est pas possible de sauter une pièce.
            ...
            // Coté gauche du pion
                ...
                if (topLeftCornerPosition != -1) {
                    Piece topleftcornerCase = pieces[topLeftCornerPosition];
                    if (topleftcornerCase == Piece.EMPTY) { // Si je peux déplacer mon pion en haut à gaucher.
                        Piece[] move = getMoveWhiteCase(pieces, srcPosition, topLeftCornerPosition);
                ...
                if (topRightCornerPosition != -1) {
                    Piece topRightcornerCase = pieces[topRightCornerPosition];
                    if (topRightcornerCase == Piece.EMPTY) { // Si je peux déplacer mon pion en haut à droite.
                        Piece[] move = getMoveWhiteCase(pieces, srcPosition, topRightCornerPosition);
                        availableMoves.add(move);
                    }
                }
    ...
    private static boolean addJumpedMoveToSequence(Piece[] pieces, int srcPosition,
        ...
        if (jumpedPosition != -1 && tgtPosition != -1) {
            ...
            // Si on peut sauter un pion adverse dans le coin supérieur gauche
                isPossibleToJump = true;
                Piece[] newCases = getJumpBlackCase(pieces, srcPosition, jumpedPosition, tgtPosition);
    ...
    private static Piece[] getJumpBlackCase(Piece[] pieces, int srcPosition,
    ...
    private static Piece[] getMoveWhiteCase(Piece[] pieces, int srcPosition, int tgtPosition) {
        assert pieces[srcPosition] != Piece.BLACK_PIECE;
        assert pieces[srcPosition] != Piece.BLACK_QUEEN;
        assert pieces[srcPosition] != Piece.EMPTY;
        assert pieces[tgtPosition] == Piece.EMPTY;
    ...
    private static List<Integer> getWhiteCases(Piece[] pieces) {
    ...
    public static Piece[] reverseCases(Piece[] pieces) {
    ...
    private static void transformPieceToQueen() {
        for(Piece[] pcs : availableMoves) {
            for (int i = 49; i >= 45; --i) {
                if (pcs[i] == Piece.WHITE_PIECE) {
                    pcs[i] = Piece.WHITE_QUEEN;
                }
            }
        }
    }
